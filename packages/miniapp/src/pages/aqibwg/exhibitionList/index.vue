...
const goDetail = (item) => {
  if (item.url) {
    ...
    navigateTo({
      url: `exhibition/index?id=${item.id}`,
    })
  }
}
...
const getNewsList = (cp) => {
  ...
  api.getExhibitionList(params).then((res) => {
    console.log('%c [ 展览 ]-50', 'font-size:13px; background:pink; color:#bf2c9f;', res)
    if (cp === 1) {
      data.value = res.list
    } else {
