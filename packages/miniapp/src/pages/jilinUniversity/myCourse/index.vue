    ...
    <view class="w-full mt-14rpx">
      ...
      >
        <n-list-cell v-for="(item, index) in data" :key="index" class="px-20rpx">
          ...
          >
            <view
              class="w-full h-280 bg-cover bg-center flex items-center"
              :style="{
                backgroundImage: `url(${
                  item?.cover || 'https://pqntourism.pcleartv.cn/Fmi-DdWPi7YA4Q2N5-zZcF20gV4d'
                })`,
              }"
            >
              <div
                v-if="!item.cover"
                class="flex-1 p-40rpx flex justify-center items-center text-#feeea9 text-40rpx text-center"
              >
                {{ item.name }}
              </div>
            </view>
            <view class="px-18rpx w-full">
              ...
              <view class="w-full flex justify-between mt-42rpx">
                ...
                <view
                  @click="goCourse(item)"
                  class="w-328 h-80 flex justify-center items-center bg-#0052D9"
                >
    ...
    <uni-popup ref="popup" type="right">
      <view class="w-510 h-100vh bg-white px-32rpx py-40rpx box-border relative">
        <view class="w-full h-full overflow-y-auto">
          ...
          <view class="w-full flex flex-wrap justify-between">
            <view
              ...
              :style="{ backgroundColor: params.courseTypeId === item.id ? '#0052D9' : '#F7F8FA' }"
          ...
          <view class="mt-87rpx w-full">
            ...
            <view class="w-full flex flex-wrap justify-between">
              <view
                ...
                :style="{ backgroundColor: currentYear === index ? '#0052D9' : '#F7F8FA' }"
          ...
          <view class="mt-87rpx w-full">
            ...
            <view class="w-full flex flex-wrap justify-between">
              <view
                ...
                :style="{
                  backgroundColor: params.courseStatus === item.status ? '#0052D9' : '#F7F8FA',
                }"
        ...
        <view class="w-full flex justify-between fixed bottom-40 left-0 px-32rpx box-border">
          ...
          <view
            @click="confirmClick"
            class="w-218 h-80 flex justify-center items-center bg-#0052D9"
          >
...
const goCourse = (item) => {
  uni.navigateTo({
    url: `/pages/jilinUniversity/course/index?id=${item.id}`,
  })
}
// ---------------------------返回该页面----------------
onShow(() => {
