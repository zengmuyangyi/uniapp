          ...
          >
            ...
            <view class="px-18rpx w-full">
              ...
              <view class="t-10rpx text-24 leading-40rpx tracking-1rpx flex items-center yahei">
                <view
                  >总学时：<span class="text-#0052D9">{{ item.classHours }}</span
                  >学时</view
                >
                <view class="ml-26rpx"
              ...
              <view class="t-10rpx text-24 leading-40rpx tracking-1rpx flex items-center yahei">
                <view
                  >课程总数：<span class="text-#0052D9">{{ item.classNums }}</span></view
                >
                <view class="ml-26rpx"
                  >已签到课程数：<span class="text-#0052D9">{{
                    item.isSignUpClassNums
                  }}</span></view
                >
              </view>
              <view
                class="w-full h-80 flex justify-center items-center mt-42rpx bg-#0052D9"
                @click="goDetail(item)"
              >
        ...
        <view class="w-full h-full overflow-y-auto">
          ...
          <view class="w-full flex flex-wrap justify-between">
            <view
              ...
              :style="{ backgroundColor: params.trainType === item.id ? '#0052D9' : '#F7F8FA' }"
          ...
          <view class="mt-87rpx w-full">
            ...
            <view class="w-full flex flex-wrap justify-between">
              <view
                ...
                :style="{
                  backgroundColor: params.trainStatus === item.status ? '#0052D9' : '#F7F8FA',
                }"
...
const goDetail = (item) => {
  uni.navigateTo({
    url: `/pages/jilinUniversity/myTraining/detail?id=${item.id}`,
  })
}
// ---------------------------返回该页面----------------
onShow(() => {
