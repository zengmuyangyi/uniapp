<template>
  <view id="exhibit" class="relative">
    <n-list v-if="optionId" bg-type="inverse" :down="null" :up="null" height="screen">
      <n-list-cell>
        <div
          @click="handleClick"
      ...
      <n-list-cell>
        <luch-audio
          v-if="getText(data.voiceUrl)"
          :autoplay="exhibitionAutoplay"
          :name="getText(data.title)"
          :src="getText(data.voiceUrl)"
          :isPlay="isExhibitionPlay"
          @onPlay="exhibitionPlay"
          audioStyle="bwg"
        ></luch-audio>
        <view class="bg-white px-30rpx w-full box-border">
...
watchEffect(() => {
  let imageList = ''
  if (getImage(data.value?.images)) {
    imageList = getImage(data.value?.coverImage) + ',' + getImage(data.value?.images)
  } else {
    imageList = getImage(data.value?.coverImage)
  }
...
onLoad((options) => {
  if (options.id) {
    ...
    fetchNews(options.id).then((res) => {
      console.log('***', res)
      data.value = res
      uni.setNavigationBarTitle({ title: getText(data.value.title) })
    })
  }
})
...
nextTick(() => {
  ...
  query
    ...
    .boundingClientRect((data) => {
      ...
      listHeight.value[0] = data.height + 150
    })
    .exec()
})
const exhibitionAutoplay = inject('exhibitionAutoplay')
const isExhibitionPlay = inject('isExhibitionPlay')
const exhibitionPlay = inject('exhibitionPlay')
</script>
