      ...
      <view id="dialogue" class="pb-40rpx">
        <view v-for="(item, index) in dialogueContent" :key="index" class="mx-30rpx">
          <view class="flex pt-20rpx" :class="item.type === 'ai' ? 'justify-start' : 'justify-end'">
            ...
            >
              <view class="leading-44rpx text-24 text-#303133" v-html="item.content"></view>
              <view
                v-for="(childItem, childIndex) in item.options || []"
                :key="childIndex"
                class="leading-44rpx text-24 text-blue-300 underline decoration-blue-300"
                :class="!childIndex ? 'mt-10rpx' : ''"
                @click="copyQuestion(childItem)"
                >{{ childIndex + 1 + '. ' + childItem.title }}</view
              >
...
const inputVal = ref('')
const aiKey = {
  donganhu: 'y0IM9g7a01GKTsyDUHFiklSSnIGj5C',
  kunshanopera: 'SHjdhzs93UUuDapUwqKniisjbitFg2',
...
const signQuest = () => {
  uni.request({
    url: `https://chatbot.weixin.qq.com/openapi/sign/${aiKey[extConfig.value.project]}`,
    data: {
...
const answerQuest = () => {
  uni.request({
    url: `https://chatbot.weixin.qq.com/openapi/aibot/${aiKey[extConfig.value.project]}`,
    data: {
