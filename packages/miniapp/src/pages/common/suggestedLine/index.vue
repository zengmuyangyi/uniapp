            ...
            >
              ...
              >
                <view class="flex items-center">
                  <view v-for="(poi, idx) in item.poi" :key="idx" class="w-200 mr-3 flex-shrink-0">
                    <view
                      class="w-full h-150 rounded-md overflow-hidden bg-gray-300 flex justify-center items-center"
                    >
                      <image
                        v-if="getImage(poi?.cover)"
                        :src="getImage(poi.cover)"
                        mode="scaleToFill"
                        class="w-full h-full"
                      />
                      <view v-else class="text-25">暂无封面</view>
                    </view>
...
import { getImage, getText, parseObject, getExtConfig } from '@/service/util'
...
const goMap = (index) => {
  ...
  if (extConfig.project === 'fanglanhu') {
    uni.navigateTo({
