...
export default class Request {
  constructor(
    ...
    this.success = successHandler || null
    this.fail = failHandler || null
    this.complete = completeHandler || null
    // interceptors
    this.requestInterceptor = reqInterceptor || null
    this.responseInterceptor = resInterceptor || null
  }

  async request(options, successHandler, failHandler, completeHandler) {
    ...
    try {
      ...
      const _res = (this.responseInterceptor &&
        this.responseInterceptor({ nError: true, response: e }, options)) || {
        nError: true,
        response: e,
      }
    ...
    if (config.nReqToCancel) {
      const _res = (this.responseInterceptor &&
        this.responseInterceptor({ nCancel: true, response: config }, options)) || {
        nCancel: true,
        response: config,
      }
      _callbackWrapper(_res, [this.fail, failHandler, this.complete, completeHandler])
