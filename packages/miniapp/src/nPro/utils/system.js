function _getSystemInfo() {
  ...
  if (info.safeAreaInsets) {
    app.globalData.xBarHeight = info.safeAreaInsets.bottom
  } else if (
    ['ios', 'devtools'].includes(info.platform) &&
    info.windowHeight / info.windowWidth > 1.78
  ) {
...
export function getRpx(val) {
  if (typeof val == 'number') {
    return getRatio() * val
  }
  if (val.indexOf('rpx') > 0 || val.indexOf('upx') > 0) {
    return parseFloat(val)
  }
  return getRatio() * parseFloat(val)
}
