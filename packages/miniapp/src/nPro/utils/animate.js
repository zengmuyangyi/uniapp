...
export function animate(ref, options, reactiveVar, extra) {
  ...
  // #endif
  // #ifndef APP-NVUE
  if (!reactiveVar) return Promise.reject('请指明响应式变量')
  ...
  sty += `transition-duration: ${options.duration || 0}ms;transition-delay: ${
    options.delay || 0
  }ms;transition-timing-function:${options.timingFunction || 'ease'};`
  reactiveVar.value = sty + (extra || '')
