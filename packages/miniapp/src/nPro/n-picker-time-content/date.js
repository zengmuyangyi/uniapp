...
const generateMonthsArr = (now, year, years, includeBefore, includeAfter) => {
  ...
  if (year * 1 !== _start * 1 && year * 1 !== _end * 1)
    return { years: years, months: generateNumArr(1, 12) }
  if (_start * 1 > _end * 1) {
    if (year * 1 === _start * 1 && !includeAfter)
      return { years: years, months: generateNumArr(now.getMonth() + 1, 1) }
    if (year * 1 === _end && !includeBefore)
      return { years: years, months: generateNumArr(12, now.getMonth() + 1) }
  } else {
    if (year * 1 === _start * 1 && !includeBefore)
      return { years: years, months: generateNumArr(now.getMonth() + 1, 12) }
    if (year * 1 === _end * 1 && !includeAfter)
      return { years: years, months: generateNumArr(1, now.getMonth() + 1) }
  }
  return { years: years, months: generateNumArr(1, 12) }
}
...
const generateDaysArr = (now, year, month, years, months, includeBefore, includeAfter) => {
  ...
  if (
    (year * 1 !== sy * 1 || month * 1 !== sm * 1) &&
    (year * 1 !== ey * 1 || month * 1 !== em * 1)
  )
    return { years: years, months: months, days: generateNumArr(1, days) }
  if (sy * 1 > ey * 1) {
    if (year * 1 === sy * 1 && month * 1 === sm * 1 && !includeAfter)
      return { years: years, months: months, days: generateNumArr(now.getDate(), 1) }
    if (year * 1 === ey * 1 && month * 1 === em * 1 && !includeBefore)
      return { years: years, months: months, days: generateNumArr(days, now.getDate()) }
  } else {
    if (year * 1 === sy * 1 && month * 1 === sm * 1 && !includeBefore)
      return { years: years, months: months, days: generateNumArr(now.getDate(), days) }
    if (year * 1 === ey * 1 && month * 1 === em * 1 && !includeAfter)
      return { years: years, months: months, days: generateNumArr(1, now.getDate()) }
  }
  return { years: years, months: months, days: generateNumArr(1, days) }
}

// 只考虑小时的includeBefore/After
const generateDateTimeArr = (
  now,
  year,
  month,
  day,
  ...
  if (ny != year * 1 || nm != month * 1 || nd != day * 1)
    return { years, months, days, hours: generateNumArr(0, 23), minutes: generateNumArr(0, 59) }
  const sy = years[0]
  const ey = years[years.length - 1]
  const sm = months[0]
  ...
  if (
    ...
    return { years, months, days, hours: generateNumArr(0, 23), minutes: generateNumArr(0, 59) }
  if (sy * 1 > ey * 1) {
    if (year * 1 === sy * 1 && month * 1 === sm * 1 && day * 1 === sd * 1 && !includeAfter)
      return { years, months, days, hours: generateNumArr(nhh, 0), minutes: generateNumArr(59, 0) }
    if (year * 1 === ey * 1 && month * 1 === em * 1 && day * 1 === ed * 1 && !includeBefore)
      return { years, months, days, hours: generateNumArr(23, nhh), minutes: generateNumArr(59, 0) }
  } else {
    if (year * 1 === sy * 1 && month * 1 === sm * 1 && day * 1 === sd * 1 && !includeBefore)
      return { years, months, days, hours: generateNumArr(nhh, 23), minutes: generateNumArr(0, 59) }
    if (year * 1 === ey * 1 && month * 1 === em * 1 && day * 1 === ed * 1 && !includeAfter)
      return { years, months, days, hours: generateNumArr(0, nhh), minutes: generateNumArr(0, 59) }
  }
  return { years, months, days, hours: generateNumArr(0, 23), minutes: generateNumArr(0, 59) }
}
...
const generateMinutesArr = (now, hour, hours, includeBefore, includeAfter) => {
  ...
  if (sh * 1 !== hour * 1 && eh * 1 !== hour * 1)
    return { hours: hours, minutes: generateNumArr(0, 59) }
  if (sh * 1 > eh * 1) {
    if (hour * 1 === sh * 1 && !includeAfter)
      return { hours, minutes: generateNumArr(now.getMinutes(), 0) }
    if (hour * 1 === eh * 1 && !includeBefore)
      return { hours, minutes: generateNumArr(59, now.getMinutes()) }
  } else {
    if (hour * 1 === sh * 1 && !includeBefore)
      return { hours, minutes: generateNumArr(now.getMinutes(), 59) }
    if (hour * 1 === eh * 1 && !includeAfter)
      return { hours, minutes: generateNumArr(0, now.getMinutes()) }
  }
  return { hours: hours, minutes: generateNumArr(0, 59) }
}
...
const findYearIndex = (years, value, now, includeBefore, includeAfter) => {
  ...
  if (yIdx < 0)
    return {
      index: [0],
      months: generateMonthsArr(now, years[0], years, includeBefore, includeAfter).months,
    }
  return {
    index: [yIdx],
    months: generateMonthsArr(now, y, years, includeBefore, includeAfter).months,
  }
}
...
const findDateIndex = (years, value, now, includeBefore, includeAfter) => {
  ...
  if (value.length <= 2)
    return {
      index: a.index.concat([0]),
      months: a.months,
      days: generateDaysArr(
        now,
        years[a.index[0]],
        a.months[a.index[1]],
        years,
...
const findTimeIndex = (hours, value, now, includeBefore, includeAfter) => {
  ...
  if (hIdx < 0)
    return {
      index: [0, 0],
      minutes: generateMinutesArr(now, hours[0], hours, includeBefore, includeAfter).minutes,
    }
  if (value.length <= 1)
    return {
      index: [hIdx, 0],
      minutes: generateMinutesArr(now, h, hours, includeBefore, includeAfter),
    }
...
const findDateTimeIndex = (years, value, now, includeBefore, includeAfter) => {
  ...
  return {
    index: a.index.concat([hIdx, mIdx]),
    months: a.months,
    days: a.days,
    hours: hms.hours,
    minutes: hms.minutes,
