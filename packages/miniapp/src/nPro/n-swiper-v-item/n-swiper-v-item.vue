...
const mrStyle = computed(() => {
  ...
  let left =
    ...
    nSwiper.itemHeightPx.value * index.value
  if (nSwiper.center.value) {
    left =
      (nSwiper.heightPx.value - nSwiper.itemHeightPx.value) * 0.5 +
      nSwiper.spacePx.value * index.value +
      nSwiper.itemHeightPx.value * index.value
  }
  style += `top:${left}px;width:${nSwiper.itemWidthPx.value}px;height:${nSwiper.itemHeightPx.value}px;`
  return style
})

function getExp(current) {
  const maxX = nSwiper.itemHeightPx.value + nSwiper.spacePx.value
  let left = -current * maxX
  const indexA = index.value
  const length = nSwiper.children.length
  if (nSwiper.circular.value) {
    left = toGetLeft(maxX, current, indexA, true)
    if (left === '') {
      return `0+${5 * maxX}`
    }
  }
...
function getScaleExp(current) {
  const sameExp = `${nSwiper.scale.value}+0`
  const maxX = nSwiper.itemHeightPx.value + nSwiper.spacePx.value
  const minusExp = `max(1 - abs(y/${maxX}), ${nSwiper.scale.value})`
  const plusExp = `min(1, ${nSwiper.scale.value}+abs(y/${maxX}))`
...
function toAdjust(x, time, init) {
  ...
  if (circle) {
    ...
    //
    if (init) {
      toInitCircle(maxX, current, indexA, iScale)
      return
    }
    // 先平移，移动之后再补充内容
    left = toGetLeft(maxX, current, indexA)
...
function toInitCircle(maxX, toCurrent, indexA, iScale) {
  const duration = nSwiper.duration.value
  const left = toGetLeft(maxX, toCurrent, indexA)
  let opacity = 0
  if (toCurrent === indexA) {
    opacity = 1
  }
...
function toSupplyCircle(maxX, current, toCurrent, length) {
  ...
  if (toLeft) {
    ...
    if (index.value === pushIdx) {
      animation.transition(
        ...
        (res) => {
          toOpacityShow(-1 * maxX * (pushIdx - 2), iScale)
        }
      )
    }
    return
...
function toGetLeft(maxX, toCurrent, indexA, isExp) {
  const length = nSwiper.children.length
  let left = maxX * toCurrent * -1
  // circle的话，至少准备5个数据
  // 不足5个数据请自行重复拼凑，不会检验
