<template>
  ...
  <div class="flex flex-wrap justify-between m-30rpx">
    ...
    >
      ...
      />
      <view
        class="flex-1 pt-40rpx pl-20rpx text-32 relative font-600"
        :style="{ color: `${props.componentStyle === 'contracted' ? block.color : 'black'}` }"
      >
        {{ block.name }}

        <view
          class="text-26 text-gray mt-8rpx font-400"
          :style="{ color: `${props.componentStyle === 'contracted' ? block.color : 'black'}` }"
          >{{ block.des }}</view
        >
      </view>
...
const blocks = computed(() => {
  const scenicInfo = extConfig.value.project === 'donganhu' ? 'ChengduToCreate' : 'EnjoyLife'
  let data = []
  if (props.componentStyle === 'contracted') {
    if (extConfig.value.project === 'liubangguli') {
      data = [
        {
          name: $t('Introduction'),
          des: $t('IntroductionDesLiu'),
        ...
        },
        {
          ...
          icon: 'https://pqntourism.pcleartv.cn/Fp04Ldq_x8n94IPkCESZ-FOCbDyv',
